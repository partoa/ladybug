...
def test_db_path_exception() -> None:
    ...
    with pytest.raises(RuntimeError, match=error_message):
        lbug.Database(path, read_only=True)


def test_read_only_exception(conn_db_readonly: ConnDB) -> None:
    _, db = conn_db_readonly
    path = db.database_path
    read_only_db = lbug.Database(path, read_only=True)
    conn = lbug.Connection(read_only_db)
...
def test_buffer_pool_size_exception() -> None:
    with pytest.raises(
        ...
        lbug.Database("test.db", buffer_pool_size=1024)
